...
// @lc code=start
class Solution {
    // 解题思路：f(chars, i, n) = f(chars, i + 1, n)  if chars[i] 属于[1 ~ 9]
    //                          + f(i + 2, n) if chars[i+1 ~ i+2] 属于 [1～26]
    public int numDecodings0(String s) {
        char[] chars = s.toCharArray();
        return numDecodings(chars, 0, chars.length - 1);
    }
    
    public int numDecodings(char[] chars, int start, int end) {
        if (start > end) return 1;
        if (chars[start] == '0') return 0;
        
        int result = numDecodings(chars, start + 1, end);
        if (start + 1 <= end) {
            int value = (chars[start] - '0') * 10 + (chars[start + 1] - '0');
            if (value <= 26)
                result += numDecodings(chars, start + 2, end);
        }
        return result;
    }
    ...
    /*
     * 解题方案：动态规划
     *        if 第i位为1～9，则合法，记剩余部分解法为count1
     *        if 第i和i+1位为10～26，也合法，记剩余部分解法为count2
     *        总解法count = count1 + count2
     * 
     * 时间复杂度：O(N) N为字符串s中字符个数
     * 空间复杂度：O(1)
     */
    public int numDecodings(String s) {
        ...
        for (int i = s.length() - 2; i >= 0; i--) {
            ...
            if (s.charAt(i) != '0') {
                count = count1; // 第i位有效
                count += (10 * (s.charAt(i) - '0') + (s.charAt(i + 1) - '0') <= 26) ? count2 : 0;
            }
