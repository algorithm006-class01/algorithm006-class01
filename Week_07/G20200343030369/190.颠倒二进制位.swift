...
class Solution {
    ...
    func reverseBitsNormal(_ n: Int) -> Int {
        var n = UInt32(n), reversed = UInt32(0)
        for _ in 0..<32 {
    ...
    func reverseBitsBits(_ n: Int) -> Int {
        var n = UInt32(n), ans = UInt32(0), bitSize = 31
        while n != 0 {
            ans += (n & 1) << bitSize
            bitSize -= 1
            n >>= 1
    ...
    func reverseBitsBitsOptimal(_ n: Int) -> Int {
        var result = 0
        var vn = n
        for i in 0...31 {
            result += (vn & 1) << (31 - i)
