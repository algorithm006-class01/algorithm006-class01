 ...
 *
 *  优秀题解（最全，各种迭代优化算法）：https://leetcode-cn.com/problems/word-ladder/solution/suan-fa-shi-xian-he-you-hua-javashuang-xiang-bfs23/
 */
...
//        int i = new LeetCode_127_379().ladderLength5("nanny", "aloud",strings  ); //结果50
    ...
    /**
     *  递归法-深度优先搜索  超时，没有成功，DSF注定比BSF耗时，不用想了
     *  执行用时 :
     *  内存消耗
     *
     *  参考题解：
   ...
   /* private void dfs(String begin, String end, List<String> wordList,Set<String> visited,int level) {
        ...
        *//***存在递归，其实不准***//*
        System.out.println("执行时间"+(end1-start));
    }*/
    private void dfs(String begin, String end, List<String> wordList,Set<String> visited,int level) {
        if(begin.equals(end)){
            if(minCount>level){
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            for (char c='a' ;c<='z';c++) {
                ...
                if(!visited.contains(next) && wordList.contains(next)){
                    ...
                    visited.add(next);
                    dfs(next,end,wordList,visited,level+1);
                    visited.remove(next);
                }
            }
            chars[i]=old;
     ...
     *
     * 执行用时 : 231 ms , 在所有 Java 提交中击败了 38.35% 的用户
     * 内存消耗 : 43.7 MB , 在所有 Java 提交中击败了 29.44% 的用户
     *
     * 参考题解：
     * https://leetcode.com/problems/word-ladder/discuss/40728/Simple-Java-BFS-solution-with-explanation
     * https://leetcode-cn.com/problems/word-ladder/solution/suan-fa-shi-xian-he-you-hua-javashuang-xiang-bfs23/ 思路5
     *
     */
    ...
    /**
     * BSF 字母拼接法2
     * 执行用时 : 109 ms , 在所有 Java 提交中击败了 45.14% 的用户
     * 内存消耗 : 44 MB , 在所有 Java 提交中击败了 29.41% 的用户
     */
    ...
    //广度算法
    /**
     * 执行用时 : 2522 ms , 在所有 Java 提交中击败了 5.02% 的用户
     * 内存消耗 : 43.9 MB , 在所有 Java 提交中击败了 28.33% 的用户
     * @param beginWord
     * @param endWord
     * @param wordList
     * @return
    ...
    public int ladderLength3(String beginWord, String endWord, List<String> wordList) {
        ...
        //由于arraylist集合匹配其中的元素慢，只能使用 hash集合
        ...
        while (!queue.isEmpty()){
            ...
            while (size-- >0){
                String poll = queue.poll();
                System.out.println("测试 k"+(++k)+"   :"+poll);
                //System.out.println("测试"+poll);
                ...
                for (String word : wordList) {
                    ...
                    for (int i = 0; i < chars.length; i++) {
                        String next = new String(chars);
                        //防止过多处理，必须用集合，因为可能存在多个值
                        ////由于arraylist集合匹配其中的元素慢，只能使用 hash集合
                        // 2、 每个结点可能包含多个子结点，可能包含多个路径，只取最短路径，所以必须用 wordlist（这里不能用arraylist，只能用hashSet类似的，否则是遍历匹配）.contains进行匹配
                        // by lyb:之前用list数组遍历，发现有问题，后取消了
    ...
    /**
     * 广度算法 成功提交版-- 差值计算法，原始版
     * 缺点：太耗时了
     * 执行用时 : 1162 ms , 在所有 Java 提交中击败了 7.25% 的用户
     * 内存消耗 : 40.8 MB , 在所有 Java 提交中击败了 46.57% 的用户
     *
     * 参考题解：https://leetcode-cn.com/problems/word-ladder/solution/suan-fa-shi-xian-he-you-hua-javashuang-xiang-bfs23/
     */
     ...
     *
     *
     * 执行用时 : 596 ms , 在所有 Java 提交中击败了 20.98% 的用户
     * 内存消耗 : 41.5 MB , 在所有 Java 提交中击败了 40.73% 的用户
     *
     * 参考题解：https://leetcode-cn.com/problems/word-ladder/solution/suan-fa-shi-xian-he-you-hua-javashuang-xiang-bfs23/
     */
    ...
    /***
     * 双向BFS，队列，visited 两个集合优化<Integer>,方便以数组方式查询数据
     * 执行用时 : 132 ms , 在所有 Java 提交中击败了 42.94% 的用户
     * 内存消耗 : 42.1 MB , 在所有 Java 提交中击败了 38.75% 的用户
     * @return
     */
        ...
        while (!queue1.isEmpty() && !queue2.isEmpty()){
            ...
            while (size-- >0){
                ...
                for (int i = 0; i < wordList.size(); i++) {
                    ...
                    if(!canConver(s,wordList.get(i))) continue;
    ...
    private boolean canConver(String next, String s) {
        int diff=0;
        for (int i = 0; i < next.length(); i++) {
            if(next.charAt(i)!=s.charAt(i)){
                if(++diff >1){
                    return false;
                }
            }
    ...
    /**
     * 耗时最短
     * 双向BSF-字母拼接法，跟ladderLength7()例子类似，只是循环部分做了小改动
     * 注意点：List<String> wordList 由于存在多个单词，涉及到contains匹配，所以必须转换Set集合，方便hash查询更快
     * 执行用时 : 23 ms , 在所有 Java 提交中击败了 89.34% 的用户
     * 内存消耗 : 42 MB , 在所有 Java 提交中击败了 39.34% 的用户
     * 参考最佳题解：第五版：https://leetcode-cn.com/problems/word-ladder/solution/suan-fa-shi-xian-he-you-hua-javashuang-xiang-bfs23/
     * @return
     */
            ...
            while (size-- >0){
                ...
                for (int i = 0; i < chars.length; i++) {
                    ...
                    for (char c='a'; c <= 'z'; c++) {
                        ...
                        if(visited2.contains(next)) return count+1;

                        //这里必须用wordSet集合，否则影响效率
