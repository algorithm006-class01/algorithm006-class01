...
//Final Best Answer: dsa-geek-qichao/i_bitwise_operation/b_n_queens_2/n_queens_2_test.go
...
func TestNQueens2(t *testing.T) {
	t.Log("N-Queens 2: DFS Prune")
	assert.Equal(t, 2, totalNQueens1(4))
	assert.Equal(t, 2, totalNQueens2(4))
}
...
func totalNQueens1(n int) int {
	...
	pies := make(map[int]bool, 2*n)
	nas := make(map[int]bool, 2*n)
	dfs1(n, row, cols, pies, nas, &res)
	return res
}

func dfs1(n int, row int, cols, pies, nas map[int]bool, res *int) {
	if row == n {
		*res++
		return
	}
	...
	for col := 0; col < n; col++ {
		...
		dfs1(n, row+1, cols, pies, nas, res)
...
func dfs2(n, row, col, pie, na int, res *int) {
	if row >= n {
		*res++
		return
	}
	...
	for bits != 0 {
		bit := bits & -bits
		dfs2(n, row+1, col|bit, (pie|bit)<<1, (na|bit)>>1, res)
		bits = bits & (bits - 1)
	}
}
